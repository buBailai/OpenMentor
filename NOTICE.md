...
## 主要新增功能（V1.1.3 / V1.1.4）

> V1.1.4 在 V1.1.3 基础上：
> - **数学 / 化学公式渲染**（V1.1.4 新）：所有 markdown 渲染面（学生对话 / 老师审计 / AI 学情报告 / 智能分析）支持 LaTeX 数学公式（`$...$` / `$$...$$`）和 mhchem 化学公式（`\ce{H2SO4}`），由 KaTeX 排版，全部资源本地化（`static/vendor/katex/` ≈ 596KB）
> - **首页「📍 打卡与意见」按钮 + 中国地图实时打卡热力图**（V1.1.4 引入）：学生 / 老师可一键打卡、看全国 OpenMentor 用户分布；「💬 提意见」跳飞书反馈表单
> - 首页 hero 区按钮单行布局微调；`.gitignore` 把数据库 `openmentor.db` 等运行时文件从 git 摘出
